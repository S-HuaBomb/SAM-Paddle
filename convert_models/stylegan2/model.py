...
from convert_models.stylegan2.op import FusedLeakyReLU, fused_leaky_relu, upfirdn2d
...
class PixelNorm(nn.Layer):
    ...
    def forward(self, input):
        return input * paddle.rsqrt(paddle.mean(input * input, 1, keepdim=True) + 1e-8)
...
class Discriminator(nn.Layer):
    ...
    def forward(self, input):
        ...
        stddev = out.reshape((group, -1, self.stddev_feat, channel // self.stddev_feat, height, width))
...
if __name__ == '__main__':
    net = Generator(1024, 512, 8)
    net.set_state_dict(paddle.load(r'D:\code_sources\VSCode\SAM_P\convert_models\paddle_models\stylegan2.pdparams'))
    net.eval()
    print(net(paddle.randn([1, 3, 512, 512], dtype='float32')))
